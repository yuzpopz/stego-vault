...
def extract_msg_logic(img_file, password_str):
    ...
    extracted_bytes = bytearray(
        int("".join(map(str, extracted_bits[i : i + 8])), 2) for i in range(0, 512, 8)
    )

    msg_len = int.from_bytes(extracted_bytes[:4], "little")
    salt, nonce, stored_mac = (
        extracted_bytes[4:20],
        extracted_bytes[20:32],
        extracted_bytes[32:64],
    )
    ...
    ciphertext = bytearray(
        int("".join(map(str, cipher_bits[i : i + 8])), 2)
        for i in range(0, len(cipher_bits), 8)
    )

    # Verify HMAC
    if not hmac.compare_digest(
        hmac.new(key, extracted_bytes[:32] + ciphertext, hashlib.sha256).digest(),
        bytes(stored_mac),
    ):
